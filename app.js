...
// Turnos: Turno 1, Turno 2, Todo el dia (configurable)
// ==========================================
let cashCloseHistoryCache = [];
let ccTurno = 'todo';
let ccHora1 = 6, ccHora2 = 14;

function initCashClose() {
    if (settings.shiftStart1) ccHora1 = settings.shiftStart1;
    if (settings.shiftStart2) ccHora2 = settings.shiftStart2;
    renderCashClose();
    loadCashCloseHistory();
}

function ccSelectTurno(t) { ccTurno = t; renderCashClose(); }

function ccUpdateHoras() {
    const h1 = document.getElementById('cc-h1');
    const h2 = document.getElementById('cc-h2');
    if (h1) ccHora1 = parseInt(h1.value.split(':')[0]) || 6;
    if (h2) ccHora2 = parseInt(h2.value.split(':')[0]) || 14;
    settings.shiftStart1 = ccHora1; settings.shiftStart2 = ccHora2;
    saveSettings(); renderCashClose();
}

function getDailyData(dateStr, turno) {
    let daySales = (sales || []).filter(s => s && s.date && s.date.substring(0, 10) === dateStr);
    if (turno === 'turno1') daySales = daySales.filter(s => { const h = new Date(s.date).getHours(); return h >= ccHora1 && h < ccHora2; });
    else if (turno === 'turno2') daySales = daySales.filter(s => { const h = new Date(s.date).getHours(); return h >= ccHora2 || h < ccHora1; });
...
function renderCashClose() {
    ...
    const d = getDailyData(hoy, ccTurno);
    ...
    const tl = ccTurno==='turno1'?'Turno 1':ccTurno==='turno2'?'Turno 2':'Todo el dia';
    const tb = (t) => ccTurno===t ? 'background:#4F46E5;color:white;border:none;box-shadow:0 3px 10px rgba(79,70,229,0.3);' : 'background:var(--card-bg);color:var(--text);border:1.5px solid var(--border);';
    container.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;"><div><h2 style="font-size:1.15rem;font-weight:800;margin:0;">🧾 Cierre de Caja</h2><p style="font-size:0.7rem;color:var(--text-muted);margin:2px 0 0;text-transform:capitalize;">'+fecha+'</p></div><span style="font-size:0.72rem;font-weight:700;color:#4F46E5;background:rgba(79,70,229,0.1);padding:4px 10px;border-radius:8px;">'+tl+'</span></div>'
    +'<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:12px;margin-bottom:12px;"><div style="display:flex;gap:6px;margin-bottom:8px;"><button onclick="ccSelectTurno(\'turno1\')" style="flex:1;padding:9px;border-radius:10px;font-size:0.78rem;font-weight:700;cursor:pointer;transition:all 0.2s;'+tb('turno1')+'">☀️ Turno 1</button><button onclick="ccSelectTurno(\'turno2\')" style="flex:1;padding:9px;border-radius:10px;font-size:0.78rem;font-weight:700;cursor:pointer;transition:all 0.2s;'+tb('turno2')+'">🌙 Turno 2</button><button onclick="ccSelectTurno(\'todo\')" style="flex:1;padding:9px;border-radius:10px;font-size:0.78rem;font-weight:700;cursor:pointer;transition:all 0.2s;'+tb('todo')+'">📊 Todo</button></div><div style="display:flex;gap:8px;font-size:0.7rem;color:var(--text-muted);"><label>☀️<input type="time" id="cc-h1" value="'+String(ccHora1).padStart(2,'0')+':00" onchange="ccUpdateHoras()" style="padding:2px 5px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text);font-size:0.7rem;width:66px;margin-left:3px;"></label><label>🌙<input type="time" id="cc-h2" value="'+String(ccHora2).padStart(2,'0')+':00" onchange="ccUpdateHoras()" style="padding:2px 5px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text);font-size:0.7rem;width:66px;margin-left:3px;"></label></div></div>'
    +'<div style="background:linear-gradient(135deg,#4F46E5,#7C3AED,#9333EA);color:white;border-radius:14px;padding:20px;margin-bottom:12px;position:relative;overflow:hidden;"><div style="position:absolute;top:-20px;right:-20px;width:90px;height:90px;background:rgba(255,255,255,0.05);border-radius:50%;"></div><div style="font-size:0.58rem;font-weight:700;letter-spacing:1.5px;opacity:0.7;">TOTAL VENDIDO — '+tl.toUpperCase()+'</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:2rem;font-weight:800;margin:4px 0;">'+formatCurrency(d.totalVentas)+'</div><div style="font-size:0.76rem;opacity:0.85;">📋 '+d.ventas+' pedidos · 📦 '+d.totalUnidades+' unidades</div></div>'
    ...
    +'<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px;"><div style="font-size:0.8rem;font-weight:700;margin-bottom:8px;">🏆 Mas vendidos</div>'+(d.top5.length===0?'<div style="color:var(--text-muted);font-size:0.78rem;">Sin ventas en este turno</div>':d.top5.map(function(p,i){return '<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid var(--border-light);"><span style="width:20px;height:20px;border-radius:5px;background:'+(i===0?'#4F46E5':i===1?'#7C3AED':'var(--border)')+';color:'+(i<2?'white':'var(--text)')+';font-size:0.65rem;font-weight:800;display:flex;align-items:center;justify-content:center;">'+String(i+1)+'</span><span style="flex:1;font-size:0.8rem;">'+esc(p[0])+'</span><span style="font-size:0.72rem;color:var(--text-muted);font-weight:600;">'+p[1]+' uds</span></div>';}).join(''))+'</div>'
    +'<div style="display:flex;gap:10px;justify-content:center;padding:12px 0;"><button class="btn btn-success" onclick="realizarCierreDeCaja()" style="padding:14px 24px;font-size:0.86rem;font-weight:700;border-radius:12px;">✅ Realizar Cierre</button><button class="btn btn-outline" onclick="printCashClose()" style="padding:12px 16px;font-size:0.8rem;border-radius:12px;">🖨️ Imprimir</button></div>'
...
function renderCashHistory(records) {
    ...
    if (!records || records.length === 0) { c.innerHTML = '<div style="text-align:center;padding:14px;color:var(--text-muted);font-size:0.78rem;">No hay cierres anteriores</div>'; return; }
    c.innerHTML = '<div style="font-size:0.85rem;font-weight:700;margin-bottom:8px;">📋 Historial de Cierres</div>' + records.map(r => '<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:10px;padding:11px 13px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;"><div><b style="font-size:0.82rem;">'+esc(r.date||'?')+'</b> <span style="font-size:0.68rem;color:var(--text-muted);background:var(--bg);padding:2px 5px;border-radius:4px;">'+esc(r.shiftName||'')+'</span><br><span style="font-size:0.7rem;color:var(--text-muted);">'+(r.salesCount||0)+' pedidos · '+esc(r.closedBy||'?')+'</span></div><div style="text-align:right;"><b style="font-family:\'JetBrains Mono\',monospace;color:var(--success);font-size:0.88rem;">'+(r.total||'$0')+'</b></div></div>').join('');
}
