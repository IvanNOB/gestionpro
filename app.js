...
function renderCashClose() {
    ...
    +'<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px;"><div style="font-size:0.8rem;font-weight:700;margin-bottom:8px;">🏆 Mas vendidos</div>'+(d.top5.length===0?'<div style="color:var(--text-muted);font-size:0.78rem;">Sin ventas hoy</div>':d.top5.map(function(p,i){return '<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid var(--border-light);"><span style="width:20px;height:20px;border-radius:5px;background:'+(i===0?'#4F46E5':i===1?'#7C3AED':'var(--border)')+';color:'+(i<2?'white':'var(--text)')+';font-size:0.65rem;font-weight:800;display:flex;align-items:center;justify-content:center;">'+String(i+1)+'</span><span style="flex:1;font-size:0.8rem;">'+esc(p[0])+'</span><span style="font-size:0.72rem;color:var(--text-muted);font-weight:600;">'+p[1]+' uds</span></div>';}).join(''))+'</div>'
